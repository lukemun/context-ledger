...
function getCurrentChangelog(changelogPath, maxLines = 100) {
  ...
  try {
    ...
    // Just get last maxLines for context
  ...
  try {
    ...
    if (versionHeaderMatch && versionHeaderMatch[1].trim()) {
      ...
      newChangelogContent = newChangelogContent.replace(versionHeaderMatch[1], '');
        ...
        // Get the full current changelog from main branch or local file
    let fullChangelog;
    try {
      fullChangelog = execSync(`git show origin/main:${changelogPath}`, { encoding: 'utf8' });
    } catch (error) {
      if (fs.existsSync(changelogPath)) {
        fullChangelog = fs.readFileSync(changelogPath, 'utf8');
      } else {
        fullChangelog = '';
      }
    }
    
    // Look for the Context Ledger marker
    const marker = '<!-- CONTEXT_LEDGER_MARKER -->';
    let finalChangelog;
    
    if (fullChangelog.includes(marker)) {
      // Insert new content before the marker
      finalChangelog = fullChangelog.replace(marker, newChangelogContent + '\n\n' + marker);
    } else {
      // No marker found, append content and add marker
      fullChangelog = fullChangelog.trimEnd();
      if (fullChangelog) {
        finalChangelog = fullChangelog + '\n\n' + newChangelogContent + '\n\n' + marker + '\n';
      } else {
    ...
    // Save the new content for the action to use
